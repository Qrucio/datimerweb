...
const UnifiedSettingsModal = ({
  ...
  isPro = false, stats = {}, onOpenPro // <--- ADDED PROP
}) => {
              ...
              <div className="w-full md:w-72 bg-[#0F0F0F] border-b md:border-b-0 md:border-r border-white/5 p-4 md:p-6 flex md:flex-col shrink-0 overflow-x-auto md:overflow-visible gap-4 md:gap-0 no-scrollbar items-center md:items-stretch">
                ...
                {user && (
                  <div className="md:mt-auto flex items-center md:flex-col gap-3 w-auto md:w-full flex-shrink-0">
                    ...
                    {/* GET PRO BUTTON (Visible if NOT Pro) */}
                    {!isPro && (
                      <button
                        onClick={onOpenPro}
                        className="w-8 h-8 md:w-full md:h-auto md:py-3 md:px-4 rounded-full md:rounded-2xl bg-gradient-to-r from-yellow-500/10 to-yellow-600/10 hover:from-yellow-500/20 hover:to-yellow-600/20 border border-yellow-500/30 flex items-center justify-center md:justify-between gap-3 group transition-all"
                        title="Upgrade to Pro"
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 md:w-6 md:h-6 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center justify-center group-hover:scale-110 transition-transform">
                            <Crown size={14} strokeWidth={2.5} />
                          </div>
                          <div className="hidden md:flex flex-col items-start">
                            <span className="text-yellow-400 text-xs font-bold uppercase tracking-widest leading-none">Get Pro</span>
                            <span className="text-yellow-500/50 text-[10px] leading-none mt-1">Unlock everything</span>
                          </div>
                        </div>
                        <div className="hidden md:block text-yellow-500/50 group-hover:text-yellow-400 group-hover:translate-x-1 transition-all">
                ...
                <AnimatePresence mode="wait">
                  ...
                  {activeTab === 'stats' && (
                    <motion.div key="stats" variants={contentVariants} initial="hidden" animate="visible" exit="exit" className="max-w-3xl pb-12 flex flex-col h-full">
                      ...
                      <AnimatePresence mode="wait">
                        {statsView === 'today' ? (
                          <motion.div key="view-today" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.15 }} className="grid grid-cols-2 gap-4">
                          ...
                          <motion.div key="view-history" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.15 }} className="flex flex-col gap-6">
                            <HistoryCalendar historyData={historyData} currentMonth={currentMonth} setCurrentMonth={setCurrentMonth} selectedDate={selectedDate} onSelectDate={setSelectedDate} isExpanded={isCalendarExpanded} setIsExpanded={setIsCalendarExpanded} />
                            <motion.div layout className="space-y-4">
                              <div className="flex items-center gap-3 border-t border-white/10 pt-6"><h4 className="font-serif-display text-lg text-white">{!isCalendarExpanded ? "Stats Overview" : selectedDate.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}</h4></div>
                              <div className="grid grid-cols-2 md:grid-cols-3 gap-4"><StatCard label="Focus Time" value={formatDuration(selectedStats.dailyFocusTime || 0)} icon={Zap} highlight /><StatCard label="Break Time" value={formatDuration(selectedStats.dailyBreakTime || 0)} icon={Coffee} /><StatCard label="Sessions" value={selectedStats.dailySessions || 0} icon={TrendingUp} /></div>
                            </motion.div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                      ...
                      {user ? (
                        <div className="bg-white/5 border border-white/10 rounded-[32px] p-10 flex flex-col items-center text-center space-y-8 relative overflow-hidden">
                          ...
                          <div className="relative transform scale-125">
                            {isPro && (<><div className="absolute -inset-[3px] rounded-full bg-gradient-to-br from-yellow-600 via-amber-400 to-yellow-600 opacity-50 blur-[4px] animate-pulse z-0" /><div className="absolute -inset-[1.5px] rounded-full overflow-hidden z-0"><div className="w-[200%] h-[200%] absolute top-[-50%] left-[-50%] animate-[spin_4s_linear_infinite]" style={{ background: 'conic-gradient(transparent 0deg, #b45309 60deg, #fcd34d 180deg, #b45309 300deg, transparent 360deg)' }} /></div></>)}
                            <div className="w-24 h-24 rounded-full border-4 border-white/10 overflow-hidden bg-black shadow-2xl relative z-10">{user.photoURL ? <img src={user.photoURL} className="w-full h-full object-cover" /> : <User size={48} className="text-white/50 m-auto h-full" />}</div>
                          </div>
                          <div className="flex flex-col items-center"><h2 className="text-3xl font-serif-display text-white">{user.displayName}</h2><p className="text-white/50 font-mono mt-1 text-sm">{user.email}</p>{isPro && <div className="mt-4 px-3 py-1 rounded-full bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2"><Sparkles size={10} /> PRO MEMBER</div>}</div>
